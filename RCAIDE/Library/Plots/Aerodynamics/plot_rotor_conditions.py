...
def plot_propulsor_data(results,distributor,propulsor,axis_1,axis_2,axis_3,axis_4,line_colors,ps,pi):
    ...
    for i in range(len(results.segments)): 
        ...
        thrust       =  bus_results[propulsor.tag][thrustor.tag].thrust[:,0]
        torque       =  bus_results[propulsor.tag][thrustor.tag].torque[:,0]
        angle        =  bus_results[propulsor.tag].commanded_thrust_vector_angle[:,0]   
        if pi == 0: 
            axis_1.plot(time,rpm, color = line_colors[i], marker = ps.markers[pi]  , linewidth = ps.line_width, label = thrustor.tag)
        else:
