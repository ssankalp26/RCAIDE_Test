            ...
            for p_i, propulsor in enumerate(bus.propulsors): 
                if p_i == 0 or bus.identical_battery_modules == False:
                    for i in range(len(results.segments)):  
                        ...
                        if p_i == 0 and i ==0:              
                            axis_1.plot(time, effp, color = line_colors[i], marker = ps.markers[bus_i], markersize= ps.marker_size, linewidth = ps.line_width, label = thrustor.tag)
                        else:
                            axis_1.plot(time, effp, color = line_colors[i], marker = ps.markers[bus_i], markersize= ps.marker_size, linewidth = ps.line_width) 
                        axis_1.set_ylim([0,1.1])
                        set_axes(axis_1)         
                         
                        axis_2.plot(time, fom, color = line_colors[i], marker = ps.markers[bus_i], markersize= ps.marker_size , linewidth = ps.line_width)
                        axis_2.set_xlabel('Time (mins)')
                        axis_2.set_ylabel(r'FoM')
                        axis_2.set_ylim([0,1.1])
                        set_axes(axis_2) 
                 
                        axis_3.plot(time, effm, color = line_colors[i], marker = ps.markers[bus_i], markersize= ps.marker_size, linewidth = ps.line_width)
