            ...
            for b_i, battery in enumerate(bus.battery_modules):
                if b_i == 0 or bus.identical_batteries == False: 
                    for i in range(len(results.segments)):  
                        ...
                        if i == 0: 
                            axis_1.plot(time, module_SOC, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag) 
                        else:
                            axis_1.plot(time, module_SOC, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width) 
                        ...
                        if i == 0: 
                            axis_2.plot(time, module_energy/Units.Wh, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag)
                        else:
                            axis_2.plot(time, module_energy/Units.Wh, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width )
                        axis_2.set_ylabel(r'Energy (W-hr)')
                        axis_2.set_xlabel('Time (mins)') 
                        set_axes(axis_2) 
                 
                        if i == 0: 
                            axis_3.plot(time, module_current, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag)
                        else:
                            axis_3.plot(time, module_current, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width )
                        axis_3.set_ylabel(r'Current (A)')
                        axis_3.set_xlabel('Time (mins)') 
                        set_axes(axis_3)
                        
                        if i == 0: 
                            axis_4.plot(time, module_power, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag)
                        else:
                            axis_4.plot(time, module_power, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width )
                        axis_4.set_ylabel(r'Power (W)')
                        axis_4.set_xlabel('Time (mins)') 
                        set_axes(axis_4)     
                         
                        if i == 0: 
                            axis_5.plot(time, module_volts, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag) 
                        else:
                            axis_5.plot(time, module_volts, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width ) 
                        axis_5.set_ylabel(r'Voltage (V)')
                        axis_5.set_xlabel('Time (mins)') 
                        set_axes(axis_5) 
                 
                        if i == 0: 
                            axis_6.plot(time, module_temperature, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width, label = battery.tag)
                        else:
                            axis_6.plot(time, module_temperature, color = line_colors[i], marker = ps.markers[b_i],markersize = ps.marker_size, linewidth = ps.line_width )
