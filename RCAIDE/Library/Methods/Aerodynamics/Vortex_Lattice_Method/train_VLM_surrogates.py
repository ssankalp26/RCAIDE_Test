...
def train_VLM_surrogates(aerodynamics):
    ...
    if len(sup_Mach) != 0: 
        ...
        aerodynamics.supersonic_surrogates =  False 
    return 
...
def train_model(aerodynamics, Mach): 
    ...
    a                    = aerodynamics.training.speed_of_sound
    training             = Data()
    training.Mach        = Mach 
    ...
    CY_alpha       = 0 * np.reshape(CY_res,(len_Mach,len_AoA)).T
    ...
    conditions.freestream.velocity                  = Machs * a
...
def new_train_model(aerodynamics, Mach): 
    """Sub function that call methods to run VLM for sample point evaluation. 
    ...
    # -------------------------------------------------------                   
    conditions                                        = RCAIDE.Framework.Mission.Common.Results()   
    conditions.frames.inertial.velocity_vector[:,0]   = u
    ...
    V_body                                            = orientation_product(T_inertial2body,conditions.frames.inertial.velocity_vector)  
    V_stability                                       = V_body * 1.  
    conditions.aerodynamics.angles.alpha[:,0]         = np.arctan2(V_stability[:,2],V_stability[:,0])
    conditions.aerodynamics.angles.beta[:,0]          = np.arctan2(V_stability[:,1],V_stability[:,0])
    conditions.aerodynamics.angles.phi[:,0]           = phi[:,0] 
    conditions.freestream.mach_number                 = np.linalg(conditions.frames.inertial.velocity_vector)[:,None] / a
    ...
    # -------------------------------------------------------     
    conditions                                        = RCAIDE.Framework.Mission.Common.Results()   
    conditions.frames.inertial.velocity_vector[:,1]   = v
    body_inertial_rotations                           = conditions.frames.body.inertial_rotations  
    phi                                               = body_inertial_rotations[:,0,None] 
    T_inertial2body                                   = angles_to_dcms(body_inertial_rotations,(2,1,0)) 
    V_body                                            = orientation_product(T_inertial2body,conditions.frames.inertial.velocity_vector)  
    ...
    # -------------------------------------------------------
    conditions                                        = RCAIDE.Framework.Mission.Common.Results()   
    conditions.frames.inertial.velocity_vector[:,2]   = w
    body_inertial_rotations                           = conditions.frames.body.inertial_rotations  
    phi                                               = body_inertial_rotations[:,0,None] 
    T_inertial2body                                   = angles_to_dcms(body_inertial_rotations,(2,1,0)) 
    V_body                                            = orientation_product(T_inertial2body,conditions.frames.inertial.velocity_vector)  
...
def train_model_new(aerodynamics, Mach): 
    """Sub function that call methods to run VLM for sample point evaluation. 
    ...
    atmo_data  = atmosphere.compute_values(aerodynamics.training.altitude)
    rho_0      = atmo_data.density
    mu_0       = atmo_data.dynamic_viscosity
    T_0        = atmo_data.temperature
    a_0        = atmo_data.speed_of_sound     
    ...
    conditions.freestream.mach_number                = Machs
    conditions.aerodynamics.angles.alpha             = np.ones_like(Machs)*AoAs 
    conditions.frames.inertial.velocity_vector[:,0]  = conditions.freestream.mach_number * a_0 
    conditions.freestream.velocity[:,0]              = conditions.freestream.mach_number * a_0  
    conditions.freestream.dynamic_viscosity          = mu_0 
    conditions.freestream.speed_of_sound             = a_0 
    conditions.freestream.temperature                = T_0 
    conditions.freestream.reynolds_number            = rho_0 * conditions.freestream.velocity / mu_0  # per m  
    conditions.freestream.dynamic_pressure           = 0.5 * rho_0 * np.sum(conditions.freestream.velocity**2, axis=1)[:,None] 
         
    Clift_res,Cdrag_res,CX_res,CY_res,CZ_res,CL_res,CM_res,CN_res, S_ref,b_ref,c_ref,X_ref,Y_ref ,Z_ref, Clift_wing_res, Cdrag_wing_res,_= evaluate_VLM(conditions,settings,vehicle)
   
    # Dimensionalize the lift and drag for each wing 
    for wing in vehicle.wings: 
        conditions.aerodynamics.coefficients.lift.inviscid_wings[wing.tag]         = Clift_wing_res[wing.tag]
        conditions.aerodynamics.coefficients.lift.compressible_wings[wing.tag]     = Clift_wing_res[wing.tag]
        conditions.aerodynamics.coefficients.drag.induced.inviscid_wings[wing.tag] = Cdrag_wing_res[wing.tag] 
    
    perturbation_state.conditions = conditions
    for wing in  vehicle.wings: 
    ...
    Cdrag_visc  = perturbation_state.conditions.aerodynamics.coefficients.drag.total
    CX_visc     = orientation_product(T_wind2inertial,Cdrag_visc)[:,0][:,None] 
    
    Clift_alpha       = np.reshape(Clift_res,(len_Mach,len_AoA)).T  
    Cdrag_alpha       = np.reshape(Cdrag_visc,(len_Mach,len_AoA)).T  
    CX_alpha          = np.reshape(CX_visc,(len_Mach,len_AoA)).T 
    CY_alpha          = 0 * np.reshape(CY_res,(len_Mach,len_AoA)).T
    CZ_alpha          = np.reshape(CZ_res,(len_Mach,len_AoA)).T 
    CL_alpha          = np.reshape(CL_res,(len_Mach,len_AoA)).T 
    CM_alpha          = np.reshape(CM_res,(len_Mach,len_AoA)).T 
    ...
    conditions.expand_rows(rows= len(Machs))
    conditions.freestream.mach_number               = Machs 
    conditions.aerodynamics.angles.alpha            = np.ones_like(Machs) *1E-20
    conditions.aerodynamics.angles.beta             = np.ones_like(Machs)*Betas   
    ...
    conditions                                       = RCAIDE.Framework.Mission.Common.Results() 
    conditions.aerodynamics.angles.alpha             = np.ones_like(Machs)*1E-20 
    conditions.frames.inertial.velocity_vector[:,0]  = Machs * a_0  +  np.atleast_2d(np.tile(u, len_Mach).T.flatten()).T 
    conditions.freestream.velocity[:,0]              = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1)
    conditions.freestream.mach_number                = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1)[:,None] / a_0 
    conditions.freestream.dynamic_viscosity          = mu_0 
    conditions.freestream.speed_of_sound             = a_0 
    conditions.freestream.temperature                = T_0 
    conditions.freestream.reynolds_number            = rho_0 * conditions.freestream.velocity / mu_0  # per m  
    ...
    Clift_u      = np.reshape(Clift_res,(len_Mach,len_u)).T - Clift_alpha_0 
    Cdrag_u      = np.reshape(Cdrag_visc_prime,(len_Mach,len_u)).T - Cdrag_alpha_0 
    CX_u         = np.reshape(CX_visc_prime,(len_Mach,len_u)).T    - CX_alpha_0  
    CY_u         = np.reshape(CY_res,(len_Mach,len_u)).T    - CY_alpha_0   
    CZ_u         = np.reshape(CZ_res,(len_Mach,len_u)).T    - CZ_alpha_0   
    CL_u         = np.reshape(CL_res,(len_Mach,len_u)).T    - CL_alpha_0   
    CM_u         = np.reshape(CM_res,(len_Mach,len_u)).T    - CM_alpha_0   
    ...
    conditions                                       = RCAIDE.Framework.Mission.Common.Results()  
    conditions.frames.inertial.velocity_vector[:,0]  = Machs * a_0  +  np.atleast_2d(np.tile(u, len_Mach).T.flatten()).T 
    conditions.frames.inertial.velocity_vector[:,1]  = np.tile(v, len_Mach).T.flatten()
    conditions.freestream.velocity[:,0]              = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1) 
    conditions.freestream.mach_number                = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1)[:,None] / a_0      
    conditions.aerodynamics.angles.alpha             = np.ones_like(Machs) *1E-20    
    conditions.freestream.reynolds_number            = rho_0 * conditions.freestream.velocity /conditions.freestream.dynamic_viscosity 
    conditions.freestream.dynamic_pressure           = 0.5 * rho_0 * np.sum(conditions.freestream.velocity**2, axis=1)[:,None] 
    ...
    conditions                                       = RCAIDE.Framework.Mission.Common.Results()  
    conditions.frames.inertial.velocity_vector[:,0]  = Machs * a_0  +  np.atleast_2d(np.tile(u, len_Mach).T.flatten()).T 
    conditions.frames.inertial.velocity_vector[:,2]  = np.tile(w, len_Mach).T.flatten()
    conditions.freestream.velocity[:,0]              = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1) 
    conditions.freestream.mach_number                = np.linalg.norm(conditions.frames.inertial.velocity_vector, axis=1)[:,None] / a_0      
    conditions.aerodynamics.angles.alpha             = np.ones_like(Machs) *1E-20    
    conditions.freestream.reynolds_number            = rho_0 * conditions.freestream.velocity /conditions.freestream.dynamic_viscosity 
    ...
    conditions.aerodynamics.angles.beta                        = np.zeros_like(Machs) 
    conditions.static_stability.roll_rate                      = np.ones_like(Machs)*p_s 
    conditions.frames.inertial.angular_velocity_vector[:,0]    = p_s
    conditions.frames.inertial.velocity_vector[:,0]            = Machs * a_0   
    conditions.freestream.velocity[:,0]                        = Machs* a_0   
    conditions.freestream.mach_number                          = Machs   
    conditions.freestream.reynolds_number                      = rho_0 * conditions.freestream.velocity /conditions.freestream.dynamic_viscosity 
    conditions.freestream.dynamic_pressure                     = 0.5 * rho_0 * np.sum(conditions.freestream.velocity**2, axis=1)[:,None]
    ...
    conditions.aerodynamics.angles.beta                        = np.zeros_like(Machs) 
    conditions.static_stability.pitch_rate                     = np.ones_like(Machs)*q_s   
    conditions.frames.inertial.angular_velocity_vector[:,1]    = q_s
    conditions.frames.inertial.velocity_vector[:,0]            = Machs * a_0   
    conditions.freestream.velocity[:,0]                        = Machs* a_0   
    conditions.freestream.mach_number                          = Machs   
    conditions.freestream.reynolds_number                      = rho_0 * conditions.freestream.velocity /conditions.freestream.dynamic_viscosity 
    ...
    Clift_q     =     np.reshape(Clift_res,(len_Mach,len_q)).T - Clift_alpha_0
    Cdrag_q     =     np.reshape(Cdrag_visc_prime,(len_Mach,len_q)).T - Cdrag_alpha_0
    CX_q        = -   np.reshape(CX_visc_prime,(len_Mach,len_q)).T    - CX_alpha_0   
    CY_q        =     np.reshape(CY_res,(len_Mach,len_q)).T    - CY_alpha_0   
    CZ_q        =     np.reshape(CZ_res,(len_Mach,len_q)).T    - CZ_alpha_0   
    CL_q        =     np.reshape(CL_res,(len_Mach,len_q)).T    - CL_alpha_0   
    CM_q        = 100*(np.reshape(CM_res,(len_Mach,len_q)).T    - CM_alpha_0)   
    ...
    conditions.aerodynamics.angles.beta                        = np.zeros_like(Machs) 
    conditions.static_stability.yaw_rate                       = np.ones_like(Machs)*r_s 
    conditions.frames.inertial.angular_velocity_vector[:,2]    = r_s
    conditions.frames.inertial.velocity_vector[:,0]            = Machs * a_0   
    conditions.freestream.velocity[:,0]                        = Machs* a_0   
    conditions.freestream.mach_number                          = Machs   
    conditions.freestream.reynolds_number                      = rho_0 * conditions.freestream.velocity /conditions.freestream.dynamic_viscosity 
