            ...
            if l == 0.0:
                ...
                for i in range(1,n):
                    ...
                    while abs(erH)>0.00001 or abs(erH1)>0.00001 or abs(erTheta)>0.00001 or abs(ercf)>0.00001:
                        ...
                        Theta[i], VeThetaH1[i] = RK4(i-1, dx, x_i, Theta, VeThetaH1, Ve_i,cf, H, dVe_i)
                        ...
                        # get skin friction
                        cf[i] = getcf(i, nu, H[i], Theta[i],Ve_i)
...
def RK4(ind, dx, x, Theta_var, VeThetaH1_var,Ve_i,cf, H, dVe_i):  
    ...
    k2 = dTheta_by_dx(ind,  x[ind] + (dx[ind]/2),  Theta_var[ind] + (k1*dx[ind]/2),  VeThetaH1_var[ind] + (l1*dx[ind]/2),Ve_i,cf, H, dVe_i)
    l2 = dVeThetaH1_by_dx(ind,  x[ind] + (dx[ind]/2),  Theta_var[ind] + (k1*dx[ind]/2),  VeThetaH1_var[ind] + (l1*dx[ind]/2),Ve_i)
    
    k3 = dTheta_by_dx(ind,  x[ind] + (dx[ind]/2),  Theta_var[ind] + (k2*dx[ind]/2),  VeThetaH1_var[ind] + (l2*dx[ind]/2),Ve_i,cf, H, dVe_i)
    l3 = dVeThetaH1_by_dx(ind,  x[ind] + (dx[ind]/2),  Theta_var[ind] + (k2*dx[ind]/2),  VeThetaH1_var[ind] + (l2*dx[ind]/2),Ve_i)
    
    k4 = dTheta_by_dx(ind,  x[ind] + dx[ind],  Theta_var[ind] + (k3*dx[ind]),  VeThetaH1_var[ind] + (l2*dx[ind]),Ve_i,cf, H, dVe_i)
    l4 = dVeThetaH1_by_dx(ind,  x[ind] + dx[ind],  Theta_var[ind] + (k3*dx[ind]),  VeThetaH1_var[ind] + (l2*dx[ind]),Ve_i)
