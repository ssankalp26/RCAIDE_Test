...
def compute_fuselage_moment_of_inertia(fuselage,center_of_gravity): 
    ...
    mass =  fuse_weight * volume_fraction[0] # mass of the hemisphere
    I =  np.zeros((3, 3)) # Local inertia tensor 
    
    # Moment of inertia in local system
    I[0][0] =  2 * mass / 5 *  (outer_radius ** 5 - inner_radius ** 5) /(outer_radius **3 -inner_radius **3) # Ixx
    I[1][1] =  2 * mass / 5 *  (outer_radius ** 5 - inner_radius ** 5) /(outer_radius **3 -inner_radius **3)# Iyy
    I[2][2] =  2 * mass / 5 *  (outer_radius ** 5 - inner_radius ** 5) /(outer_radius **3 -inner_radius **3) # Izz

    # global system
    s = np.array(origin_hemisphere) - np.array(center_of_gravity)
    I_global = np.array(I) + mass * (np.array(np.dot(s[0], s[0])) * np.array(np.identity(3)) - s*np.transpose(s)) # global inertia tensor for hemisphere
    ...
    mass = fuse_weight *volume_fraction[1] #mass_of_component_cylinder
    ...
    # Moment of inertia in local system
    I[0][0] =  mass / 2 *  (outer_radius ** 2 + inner_radius ** 2) # Ixx
    I[1][1] =  mass / 12 * (3 * (outer_radius ** 2 + inner_radius ** 2) + center_length ** 2) # Iyy
    I[2][2] =  mass / 12 * (3 * (outer_radius ** 2 + inner_radius ** 2) + center_length ** 2) # Izz
    ...
    mass = fuse_weight *volume_fraction[2]
    origin_cone =  np.array([fuselage.lengths.nose + center_length / 2,0, 0]) + np.array(fuselage.origin)
    ...
    # Moment of inertia in local system
    # ----------------------------------------------------------------------------------------------------------------------    
    rho = (mass / (1 / 3 * np.pi * (outer_radius ** 2 * center_length - inner_radius ** 2 * (center_length * inner_radius / outer_radius)))) # density of the cone. Mass divided by volume.
