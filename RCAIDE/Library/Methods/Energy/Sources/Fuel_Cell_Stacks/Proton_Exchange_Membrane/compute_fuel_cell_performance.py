...
def compute_fuel_cell_performance(fuel_cell_stack,state,bus,coolant_lines,t_idx,delta_t): 
    ...
    fuel_cell_mass  = fuel_cell.mass_properties.mass
    ...
    fuel_cell_conditions = fuel_cell_stack_conditions.fuel_cell 
    ...
    Q_heat_fuel_cell   = fuel_cell_stack_conditions.fuel_cell.heat_energy_generated 
    V_ul_fuel_cell     = fuel_cell_stack_conditions.fuel_cell.voltage_under_load 
    I_cell             = fuel_cell_stack_conditions.fuel_cell.current      
    T_cell             = fuel_cell_stack_conditions.fuel_cell.temperature 
    
    P_stack            = fuel_cell_stack_conditions.power
    Q_heat_stack       = fuel_cell_stack_conditions.heat_energy_generated
    V_ul_stack         = fuel_cell_stack_conditions.voltage_under_load
    I_stack            = fuel_cell_stack_conditions.current 
    T_stack            = fuel_cell_stack_conditions.temperature           
    ...
    while abs(diff_V) < 1E-12:
        ...
        fuel_cell_conditions.pem.P_H2_input[t_idx] =  P_H2_and_air * FAR
        fuel_cell_conditions.pem.P_air[t_idx]      =  (1 - FAR) * P_H2_and_air
...
def evaluate_PEM(fuel_cell_stack,fuel_cell_conditions,t_idx):
    ...
    """
    fuel_cell = fuel_cell_stack.fuel_cell
    i         =  fuel_cell_conditions.pem.i[t_idx]  # input 
    lambda_O2 =  fuel_cell_conditions.pem.lambda_O2[t_idx] # known  confirmed 
    
    fuel_cell_conditions.pem.p_drop_fc[t_idx] = calculate_P_drop_stack(fuel_cell,i)
    if fuel_cell.type == "LT":
        fuel_cell_conditions.pem.p_drop_hum[t_idx] = calculate_P_drop_hum(fuel_cell,i) 
    else: 
        fuel_cell_conditions.pem.p_drop_hum[t_idx] = 0 
    
    mdot_air_in     = i * fuel_cell.A * fuel_cell.MMO2 / (4 * fuel_cell.F * fuel_cell.O2_mass_frac) * lambda_O2
...
def evaluate_CEM(fuel_cell_stack,fuel_cell_conditions,t_idx):
    ...
    """
    fuel_cell  = fuel_cell_stack.fuel_cell
    FC_air_p   = fuel_cell_conditions.pem.FC_air_p[t_idx]
    p_drop_hum = fuel_cell_conditions.pem.p_drop_hum[t_idx]  # calculated 
    fuel_cell_conditions.cem.p_air_out[t_idx]  =  FC_air_p  + p_drop_hum
    
    evaluate_CEM_model(fuel_cell.CEM,fuel_cell_conditions,t_idx)
    power_req      = fuel_cell_conditions.cem.P_CEM[t_idx]
    mdot_out       = fuel_cell_conditions.cem.mdot_out_exp[t_idx]
    expander_power = fuel_cell_conditions.cem.P_exp[t_idx]
...
def calculate_voltage(i, fuel_cell_stack,fuel_cell_conditions,t_idx):
    ...
    fuel_cell   = fuel_cell_stack.fuel_cell
    #i           = fuel_cell_conditions.pem.i[t_idx]   # input 
    T_fc        = fuel_cell_conditions.pem.T_fc[t_idx, 0]        # known confirmed 
    P_H2_input  = fuel_cell_conditions.pem.P_H2_input[t_idx, 0]   
    P_air       = fuel_cell_conditions.pem.P_air[t_idx, 0]      
    RH          = fuel_cell_conditions.pem.RH[t_idx, 0]           # known  confirmed    
    lambda_O2   = fuel_cell_conditions.pem.lambda_O2[t_idx, 0]     # known  confirmed 
    p_drop_fc   = fuel_cell_conditions.pem.p_drop_fc[t_idx, 0]      # known 
    degradation = fuel_cell_conditions.pem.degradation[t_idx, 0]    # known  confirmed  
    
    P_O2   = calculate_P_O2(fuel_cell_stack,P_air, T_fc, RH, lambda_O2, p_drop_fc, i)
    P_H2   = calculate_P_H2(fuel_cell_stack,P_H2_input, T_fc, RH, i)
    E_cell = calculate_E_cell(fuel_cell_stack,T_fc, P_H2, P_O2)
    
    if fuel_cell.type == "LT": 
        eta_ohmic  = calculate_ohmic_losses_LT(fuel_cell_stack,T_fc, i) 
        eta_conc   = calculate_concentration_losses_LT(fuel_cell_stack,T_fc, P_O2, RH, lambda_O2, p_drop_fc, i)
    elif fuel_cell.type == "HT": 
        eta_ohmic = calculate_ohmic_losses_HT(fuel_cell_stack,T_fc, i) 
        eta_conc  = calculate_concentration_losses_HT(fuel_cell_stack,T_fc, P_O2, RH, lambda_O2, p_drop_fc, i) 
    eta_act = calculate_activation_losses(fuel_cell_stack,T_fc, P_O2, i) 
...
def calculate_P_O2(fuel_cell_stack, P_air, T_fc, RH, lambda_O2, P_drop, i): 
    """
...
def calculate_P_H2(fuel_cell_stack, P_H2_input, T_fc, RH, i):
    """
...
def calculate_E_cell(fuel_cell_stack, T_fc, P_H2, P_O2):
    """
    ...
    try:
        ...
        fuel_cell.R*T_fc / (4 * fuel_cell.alpha * fuel_cell.F) * (np.log(P_H2) + 0.5 * np.log(P_O2))
    except: 
        return -10
    return E_cell

def calculate_activation_losses(fuel_cell_stack, T_fc, P_O2, i):
    """
    ...
    """
    fuel_cell = fuel_cell_stack.fuel_cell
    A_const   = fuel_cell.R * T_fc / (2 * fuel_cell.alpha * fuel_cell.F)
    i0        = fuel_cell.i0ref * fuel_cell.L_c * fuel_cell.a_c * (P_O2/fuel_cell.i0ref_P_ref) ** (fuel_cell.gamma) * \
        np.exp(-fuel_cell.E_C / (fuel_cell.R * T_fc) * (1 - (T_fc / fuel_cell.i0ref_T_ref)))
    eta_act = A_const * np.log(i/i0)
    return eta_act

def calculate_ohmic_losses_LT(fuel_cell_stack, T_fc, i):
    """
...
def calculate_ohmic_losses_HT(fuel_cell_stack, T_fc, i):
    """
...
def calculate_concentration_losses_LT(fuel_cell_stack, T_fc, P_O2, RH, lambda_O2, P_drop, i):
    """
    ...
    """
    fuel_cell =  fuel_cell_stack.fuel_cell
    i_lim =  calculate_limiting_current_density_LT(fuel_cell_stack, T_fc, P_O2, RH, lambda_O2, P_drop, i)
    if i >= i_lim: 
        return 10
    else: 
        eta_conc = (1 + 1 / fuel_cell.alpha) * fuel_cell.R * T_fc / (2 * fuel_cell.F) * np.log(i_lim / (i_lim - i)) 
        return eta_conc
    
def calculate_limiting_current_density_LT(fuel_cell_stack, T_fc, P_O2, RH, lambda_O2, P_drop, i, **kwargs): 
    """
    ...
    """
    fuel_cell        =  fuel_cell_stack.fuel_cell
    P_O2_ref_1_atm   = calculate_P_O2(fuel_cell_stack, 1, T_fc, RH, lambda_O2, P_drop, i)
    P_O2_ref_2_5_atm = calculate_P_O2(fuel_cell_stack, 2.5, T_fc, RH, lambda_O2, P_drop, i)
    i_lim = (2.25 - 1.65) * (P_O2 - P_O2_ref_1_atm)**1 / (P_O2_ref_2_5_atm-P_O2_ref_1_atm)**1 + 1.65
    return fuel_cell.i_lim_multiplier * i_lim
    
def calculate_concentration_losses_HT(fuel_cell_stack, T_fc, P_O2, RH, lambda_O2, P_drop, i):
    """
    ...
    """
    fuel_cell = fuel_cell_stack.fuel_cell
    i_lim = calculate_limiting_current_density_HT(fuel_cell_stack,T_fc, P_O2, RH, lambda_O2, P_drop, i)
    if i >= i_lim: 
        return 10
    else: 
        eta_conc = (1 + 1.8/fuel_cell.alpha) * fuel_cell.R * T_fc / (2* fuel_cell.F) * np.log(i_lim / (i_lim - i)) 
        return eta_conc

def calculate_limiting_current_density_HT(fuel_cell_stack, T_fc, P_O2, RH, lambda_O2, P_drop, i): 
    """
    ...
    fuel_cell      = fuel_cell_stack.fuel_cell
    P_O2_ref_1_atm = calculate_P_O2(fuel_cell_stack, 1, T_fc, RH, lambda_O2, P_drop, i)
    P_O2_ref_2_atm = calculate_P_O2(fuel_cell_stack,2, T_fc, RH, lambda_O2, P_drop, i)
...
def evaluate_max_gross_power(fuel_cell_stack,fuel_cell_conditions,t_idx):
    ...
    fuel_cell    = fuel_cell_stack.fuel_cell
    FC_air_p     = fuel_cell_conditions.pem.FC_air_p[t_idx, 0]    
    T_fc         = fuel_cell_conditions.pem.T_fc[t_idx, 0]        
    RH           = fuel_cell_conditions.pem.RH[t_idx, 0]          
    lambda_O2    = fuel_cell_conditions.pem.lambda_O2[t_idx, 0] 
     
    P_O2 = calculate_P_O2(fuel_cell_stack,FC_air_p, T_fc, RH, lambda_O2, fuel_cell.rated_p_drop_fc, 0) 

    if fuel_cell.type == "LT":
        i_lim = calculate_limiting_current_density_LT(fuel_cell_stack,T_fc, P_O2, RH, lambda_O2, 0, i=0)
    elif fuel_cell.type == "HT":
        i_lim = calculate_limiting_current_density_HT(fuel_cell_stack,T_fc, P_O2, RH, lambda_O2, 0, i=0)  
...
def evaluate_max_net_power(fuel_cell_stack,fuel_cell_conditions,t_idx): 
    ...
    fuel_cell   = fuel_cell_stack.fuel_cell     
    FC_air_p     = fuel_cell_conditions.pem.FC_air_p  
    T_fc         = fuel_cell_conditions.pem.T_fc      
    RH           = fuel_cell_conditions.pem.RH        
    lambda_O2    = fuel_cell_conditions.pem.lambda_O2
...
def generate_gross_polarization_curve_data(fuel_cell_stack,fuel_cell_conditions,t_idx,  no_points = 200):
    ...
    """
    i = fuel_cell_conditions.pem.i[t_idx,0]  # Current densities from 0 to 2 A/cm^2 in steps of 0.01 A/cm^2 
    fuel_cell_conditions.pem.p_drop_fc[t_idx] =  calculate_P_drop_stack(fuel_cell_stack,i)
...
def evaluate_CEM_model(CEM,fuel_cell_conditions, t_idx): 
    Tt_in          = fuel_cell_conditions.cem.stagnation_temperature[t_idx] 
    Pt_in          = fuel_cell_conditions.cem.stagnation_presure[t_idx] 
    mdot_air_in    = fuel_cell_conditions.cem.mdot_air_in[t_idx]
    p_air_FC       = fuel_cell_conditions.pem.p_air_out[t_idx] 
    p_drop_hum     = fuel_cell_conditions.pem.p_drop_hum[t_idx]
    p_drop_fc      = fuel_cell_conditions.pem.p_drop_fc[t_idx]
    lambda_O2      = fuel_cell_conditions.pem.lambda_O2[t_idx] 
    ...
    fuel_cell_conditions.cem.p_in_comp[t_idx]        = Pt_in 
    fuel_cell_conditions.cem.pi_comp[t_idx]          = (p_air_FC + p_drop_hum) / Pt_in 
    fuel_cell_conditions.cem.mdot_in_comp[t_idx]     = mdot_air_in 
    fuel_cell_conditions.cem.mdot_out_exp[t_idx]     = mdot_air_out 
    fuel_cell_conditions.cem.p_in_exp[t_idx]         = p_exp 
    fuel_cell_conditions.cem.pi_exp[t_idx]           = Pt_in / p_exp 
    fuel_cell_conditions.cem.P_comp[t_idx]           = comp_p_req
    fuel_cell_conditions.cem.P_motor_comp[t_idx]     = input_p
    fuel_cell_conditions.cem.P_exp[t_idx]            = exp_p_ext
    fuel_cell_conditions.cem.P_generator_exp[t_idx]  = output_p 
    fuel_cell_conditions.cem.P_CEM[t_idx]            = p_req
    return  
