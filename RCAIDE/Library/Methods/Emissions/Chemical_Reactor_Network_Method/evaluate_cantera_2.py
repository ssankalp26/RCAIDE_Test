...
def combustor_model(kinetics_model,oxidizer_model,high_fidelity_kin_mech, dict_fuel, dict_oxy, T_stag_0, P_stag_0, FAR, m_dot_fuel, m_dot_air, N_PZ, A_PZ, L_PZ, phi_sign, phi_SZ, A_SZ, L_SZ, f_air_PZ, N_SZ, sigma_phi):
    ...
    for _ in  range(N_PFR): 
        ...
        outlet_air            = ct.MassFlowController(mixer_air, PFR, mdot=m_dot_air_1 ) # [-]       Connect the mixer with the downstream PFR with the same mass flow rate           
        ...
        mixer_air_2           = ct.IdealGasReactor(Fuel_list[0])                            # [-]       Create the reactor for the downstream mixer
        outlet_PFR            = ct.MassFlowController(PFR, mixer_air_2, mdot=m_dot_air_1 ) # [-]       Connect the PFR with the downstream mixer                                 
        t_res_PFR             = (Fuel_list[0].density * PFR.volume) / (m_dot_air_1)       # [s]       Compute the residence time inside the PFR           
        ...
        mixer_air =  mixer_air_2
        total_mass_flow = m_dot_air_1
    # ------------------------------------------------------------------------------
