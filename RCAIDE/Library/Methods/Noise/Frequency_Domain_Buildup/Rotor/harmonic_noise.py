...
def harmonic_noise(harmonics,freestream,angle_of_attack,coordinates,
    ...
    m              = np.tile(harmonics[None,None,None,:],(num_cpt,num_mic,num_sec,1))                 # harmonic number 
    m_1d           = harmonics                                                                                         
    p_ref          = 2E-5                                                                                        # referece atmospheric pressure
    a              = np.tile(freestream.speed_of_sound[:,:,None,None],(1,num_mic,num_sec,num_h))      # speed of sound
    rho            = np.tile(freestream.density[:,:,None,None],(1,num_mic,num_sec,num_h))             # air density   
    alpha          = np.tile((angle_of_attack + np.arccos(body2thrust[0,0]))[:,:,None,None],(1,num_mic,num_sec,num_h))          
    B              = rotor.number_of_blades                                                                      # number of rotor blades
    omega          = np.tile(aeroacoustic_data.omega[:,:,None,None],(1,num_mic,num_sec,num_h))        # angular velocity       
    dT_dr          = np.tile(aeroacoustic_data.blade_dT_dr[:,None,:,None],(1,num_mic,1,num_h))      # nondimensionalized differential thrust distribution 
    dQ_dr          = np.tile(aeroacoustic_data.blade_dQ_dr[:,None,:,None],(1,num_mic,1,num_h))      # nondimensionalized differential torque distribution
    R              = np.tile(rotor.radius_distribution[None,None,:,None],(num_cpt,num_mic,1,num_h)) # radial location     
    c              = np.tile(rotor.chord_distribution[None,None,:,None],(num_cpt,num_mic,1,num_h))  # blade chord    
    ...
    Y              = np.tile(np.sqrt(coordinates.X_hub[:,:,0,:,1]**2 +  coordinates.X_hub[:,:,0,:,2] **2)[:,:,:,None],(1,1,1,num_h))                        # observer distance from rotor axis          
    V              = np.tile(np.linalg.norm(velocity_vector,axis =1) [:,None,None,None],(1,num_mic,num_sec,num_h))                                                     # velocity magnitude
    M_x            = V/a                                                                                         
    ...
    psi_V          = np.zeros((num_cpt,num_mic,num_sec,num_h))
