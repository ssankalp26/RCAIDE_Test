...
def broadband_noise_unsteady(freestream,angle_of_attack,coordinates,
    ...
    if np.all(Omega == 0):
        ...
        c                 = np.tile(blade_chords[None,None,None,:,None,None],(num_mic,num_rot,num_blades,1,num_cf,num_az))
        L                 = np.tile(L[None,None,None,:,None,None],(num_mic,num_rot,num_blades,1,num_cf,num_az))
        f                 = np.tile(frequency[None,None,None,None,:,None],(num_mic,num_rot,num_blades,num_sec,1,num_az))
            
        SPLb_sum_3 = np.zeros((num_cpt,num_mic,num_rot,num_blades,num_cf))
        for cpt_i in range(num_cpt):
            alpha_blade       = np.tile(alpha[cpt_i,None,None,None,:,None,:],(1,num_mic,num_rot,num_blades,1,num_cf,1))
            V                 = np.zeros((num_mic,num_rot,num_blades,num_sec,num_cf,num_az,3))
            V[:,:,:,:,:,:,0]  = -np.tile(Vt[cpt_i,None,None,None,:,None,None],(1,num_mic,num_rot,num_blades,1,num_cf,num_az)) 
            V[:,:,:,:,:,:,2]  = np.tile(Va[cpt_i,None,None,None,:,None,None],(1,num_mic,num_rot,num_blades,1,num_cf,num_az))  
            V_tot             = np.linalg.norm(V, axis=6)
            alpha_tip         = np.tile(alpha_tip[cpt_i,None,None,None,None,:],(1,num_mic,num_rot,num_blades,num_sec,num_cf,1))  
            c_0               = np.tile(speed_of_sound[cpt_i,:,None,None,None,None,None],(1,num_mic,num_rot,num_blades,num_sec,num_cf,num_az))
            rho               = np.tile(density[cpt_i,:,None,None,None,None,None],(1,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)) 
            mu                = np.tile(dyna_visc[cpt_i,:,None,None,None,None,None],(1,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)) 
            R_c               = np.tile(blade_Re[cpt_i,None,None,None,:,None,:],(1,num_mic,num_rot,num_blades,1,num_cf,1))
            U                 = np.tile(blade_speed[cpt_i,None,None,None,:,None,:],(1,num_mic,num_rot,num_blades,1,num_cf,1))
            M                 = np.tile(blade_Ma[cpt_i,None,None,None,:,None,:],(1,num_mic,num_rot,num_blades,1,num_cf,1)) # U/c_0 
            M_tot             = V_tot/c_0   
              
            X_prime_r         = np.tile(coordinates.X_prime_r[cpt_i,:,:,:,:,None,None,:],(1,1,1,1,num_cf,num_az,1))
            cos_zeta_r        = np.sum(X_prime_r*V, axis = 5)/(np.linalg.norm(X_prime_r, axis = 5)*np.linalg.norm(V, axis = 5)) 
            r_er              = np.tile(np.linalg.norm(coordinates.X_e_r, axis = 5)[cpt_i,:,:,:,:,None,None],(1,1,1,1,num_cf,num_az))           
            Phi_er            = np.tile(coordinates.phi_e_r[cpt_i,:,:,:,:,None,None],(1,1,1,1,num_cf,num_az))
            Theta_er          = np.tile(coordinates.theta_e_r[cpt_i,:,:,:,:,None,None],(1,1,1,1,num_cf,num_az))    
            
            # flatten matrices 
            R_c        = flatten_matrix(R_c,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            c          = flatten_matrix(c,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            alpha_star = flatten_matrix(alpha_blade,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            alpha_tip  = flatten_matrix(alpha_tip,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            U          = flatten_matrix(U,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            cos_zeta_r = flatten_matrix(cos_zeta_r,num_mic,num_rot,num_blades,num_sec,num_cf,3)
            M_tot      = flatten_matrix(M_tot,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            f          = flatten_matrix(f,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            c_0        = flatten_matrix(c_0,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            rho        = flatten_matrix(rho,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            r_er       = flatten_matrix(r_er,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            mu         = flatten_matrix(mu,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            L          = flatten_matrix(L,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            M          = flatten_matrix(M,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            Phi_er     = flatten_matrix(Phi_er,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
            Theta_er   = flatten_matrix(Theta_er,num_mic,num_rot,num_blades,num_sec,num_cf,num_az)
...
# def flatten_matrix(x,num_mic,num_rot,num_blades,num_sec,num_cf,num_az):
#     return np.reshape(x,(num_mic*num_rot*num_blades*num_sec*num_cf*num_az))
...
def unflatten_matrix(x,num_mic,num_rot,num_blades,num_sec,num_cf,num_az):
    return np.reshape(x,(num_mic,num_rot,num_blades,num_sec,num_cf,num_az))