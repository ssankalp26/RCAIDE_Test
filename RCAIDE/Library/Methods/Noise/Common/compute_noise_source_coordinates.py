...
def compute_rotor_point_source_coordinates(distributor,propulsor,rotor,conditions,mls,settings):
    ...
    Rotation_AoA[:,:,:,:,0:3,0:3]       = conditions.frames.body.transform_to_inertial[:,np.newaxis,np.newaxis,np.newaxis,:,:]  
    ...
    Translation_XYZ[:,:,:,:,0,3]   = np.tile(mls[:,:,0][:,:,None,None],(1,1,num_blades,num_sec)) 
    Translation_XYZ[:,:,:,:,1,3]   = np.tile(mls[:,:,1][:,:,None,None],(1,1,num_blades,num_sec)) 
    Translation_XYZ[:,:,:,:,2,3]   = np.tile(mls[:,:,2][:,:,None,None],(1,1,num_blades,num_sec))    
