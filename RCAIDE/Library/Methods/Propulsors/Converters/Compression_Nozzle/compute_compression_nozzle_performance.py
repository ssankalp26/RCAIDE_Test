...
def compute_compression_nozzle_performance(compression_nozzle,nozzle_conditions,conditions):
    ...
    P0                      = nozzle_conditions.inputs.static_temperature
    T0                      = nozzle_conditions.inputs.static_pressure   
    ...
    if compressibility_effects: 
        ...
        if Mach <= 1.0: # use isentropic relations
            i_low          = M0 <= 1.0
            ...
            P_out[i_low]   = Pt_out[i_low]/((1.+(gamma[i_low]-1.)/2.*Mach[i_low]*Mach[i_low])**(gamma[i_low]/(gamma[i_low]-1.)))
        ...
        P_out  = Pt_out/(1.+(gamma-1.)/2.*Mach*Mach)**(gamma/(gamma-1.))
    ...
    nozzle_conditions.outputs.mach_number             = M0
